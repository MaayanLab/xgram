...
def find_groups():
	...
	X = scipy.randn(10, 2)     # 100 2-dimensional observations
	d = hier.distance.pdist(X)   # vector of (100 choose 2) pairwise distances
	L = hier.linkage(d, method='complete')
	# ind = hier.fcluster(L, 0.5,'inconsistent')

	all_dist  = [0,0.25,0.5,0.75,1.0]

	for inst_dist in all_dist:
		print(hier.fcluster(L, inst_dist*d.max(), 'distance'))
		print('\n')
