...
function load_chik_cutoff(inst_cutoff){
	...
	d3.json('/ChikungunyaUSAMRIID/static/networks/chik_cutoff_'+inst_cutoff+'.json', function(network_data){
...
function load_new_clustergram(){
		...
		// // switch view: remove class and zscore buttons 
		// d3.select('#class_buttons').style('display','inline');
		// d3.select('#zscore_buttons').style('display','inline');
	...
	d3.json('/ChikungunyaUSAMRIID/static/networks/'+ccle_params['gc'].toLowerCase()+'_exp_std_'+ccle_params['cutoff']+'.json', function(network_data){
...
// all of this is taken care of in d3_clustergram.js
/////////////////
